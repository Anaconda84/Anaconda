...
def run_test(params = None):
    ...
    if len(videofiles) > 1:
        print >>sys.stderr,time.asctime(),'-', "main: More then one video file found!!"
    else:
    ...
    if tdef.is_multifile_torrent():
        ...
        if dlfiles is not None:
            print >>sys.stderr,time.asctime(),'-', "main: Found SVC video!!"
    ...
    print >>sys.stderr,time.asctime(),'-', "main: Saving content to", d.get_dest_files()
