...
videoextdefaults = ['aac','asf','avi','dv','divx','flac','flc','flv','mkv','mpeg','mpeg4','mpegts','mpg4','mp3','mp4','mpg','mkv','mov','m4v','ogg','ogm', 'ogv', 'oga', 'ogx', 'qt','rm','swf','vob','wmv','wav']
...
def quote_program_path(progpath):
    ...
    if idx != -1:
        ...
        if not os.access(progpath,os.R_OK):
            if DEBUG:
                print >>sys.stderr,time.asctime(),'-', "videoplay: Could not find assumed progpath",progpath
            return None
        return '"'+progpath+'"'
    else:
        return progpath
