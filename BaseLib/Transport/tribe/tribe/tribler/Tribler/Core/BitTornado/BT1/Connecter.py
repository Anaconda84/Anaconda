...
class Connection:
    ...
    def send_bitfield(self, bitfield):
        if self.can_send_to():
            ...
            print >>sys.stderr,time.asctime(),'-', "Sending empty bitfield to unauth node"
    ...
    def send_extend_handshake(self):
        ...
        self._send_message(EXTEND + EXTEND_MSG_HANDSHAKE_ID + bencode(d))
        if DEBUG:
            print >>sys.stderr,time.asctime(),'-', 'connecter: sent extend: id=0+',d
    ...
    def _send_cs_message(self, cs_list):
        if DEBUG_CS:
            print >>sys.stderr,time.asctime(),'-', "connecter: conn: _send_cs_message",cs_list[0]
    ...
    def got_cs_message(self, cs_list):
        if DEBUG_CS:
            print >>sys.stderr,time.asctime(),'-', "connecter: conn: got_cs_message",cs_list[0]
...
# 2fastbt_
    ...
    def connection_made(self, connection):
        ...
        if connection.supports_extend_messages():
            ...
            if DEBUG:
                print >>sys.stderr,time.asctime(),'-', "connecter: Peer is client",client,"version",version,c.get_ip()
    ...
    def _cs_completed(self, connection):
        ...
        try:
            ...
            print >> sys.stderr,time.asctime(),'-', "Error restarting after CS handshake:",e
