...
def get_tree_height(npieces):
    if DEBUG:
        print >> sys.stderr,time.asctime(),'-', "merkle: number of pieces is",npieces
    ...
    if height - floor(height) > 0.0:
        ...
        height = int(height)
    if DEBUG:
        print >> sys.stderr,time.asctime(),'-', "merkle: tree height is",height
    return height
...
def create_tree(height):
    ...
    if DEBUG:
        print >> sys.stderr,time.asctime(),'-', "merkle: treesize",treesize
...
def get_hashes_for_piece(tree,height,index):
    ...
    if DEBUG:
        print >> sys.stderr,time.asctime(),'-', "merkle: myoffset",myoffset
    ...
    if myoffset % 2 == 0:
        ...
        siblingoffset = myoffset+1
    if DEBUG:
        print >> sys.stderr,time.asctime(),'-', "merkle: siblingoffset",siblingoffset
    if siblingoffset != -1:
    ...
    for level in range(height,0,-1):
        ...
        if DEBUG:
            print >> sys.stderr,time.asctime(),'-', "merkle: uncleoffset",uncleoffset
...
def update_hash_admin(hashlist,tree,height,hashes):
    ...
    for i in range(0,len(hashlist)):
        if i < 2:
            ...
            if index < len(hashes):
                if DEBUG:
                    print >> sys.stderr,time.asctime(),'-', "merkle: update_hash_admin: saving hash of",index
