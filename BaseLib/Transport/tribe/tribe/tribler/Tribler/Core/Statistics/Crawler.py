...
#         if DEBUG: print >>sys.stderr, time.asctime(),'-', "crawler: connecting... (send_reply)", show_permid_short(permid)
    ...
    def handle_connection(self, exc, permid, selversion, locally_initiated):
        ...
        if exc:
            # connection lost
            if DEBUG: print >>sys.stderr, time.asctime(),'-', "crawler: overlay connection lost", show_permid_short(permid), exc
            ...
            if not already_known:
                if DEBUG: print >>sys.stderr, time.asctime(),'-', "crawler: new overlay connection", show_permid_short(permid)
            ...
            if DEBUG: print >>sys.stderr, time.asctime(),'-', "crawler: new overlay connection (can not use version %d)" % selversion, show_permid_short(permid)
