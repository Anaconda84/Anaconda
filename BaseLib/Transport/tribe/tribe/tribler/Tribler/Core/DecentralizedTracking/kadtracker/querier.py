...
class Querier(object):
    ...
    def send_query(self, msg, node_, on_response_f,
        ...
        tid = self._next_tid()
        logging.debug('sending to node: %r\n%r' % (node_, msg))
    ...
    def on_response_received(self, response_msg, addr):
        ...
        for query_index, query in enumerate(addr_query_list):
            ...
            if query.matching_tid(response_msg.tid):
                ...
                break
        if not query_found:
            logging.warning('No query for this response\n%s\nsource: %s' % (
                response_msg, addr))
            return # ignore response 
        ...
        if response_is_ok:
            ...
            logging.warning('Bad response from %r\n%r' % (addr,
                                                          response_msg))

    def on_error_received(self, error_msg, addr):
        logging.warning('Error message received:\n%s\nSource: %s',
                        `error_msg`,
                        `addr`)
