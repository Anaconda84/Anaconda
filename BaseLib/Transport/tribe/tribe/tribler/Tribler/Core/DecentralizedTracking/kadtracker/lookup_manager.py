...
class _LookupQueue(object):
    ...
    def _remove_furthest_node(self):
        logging.debug('highest_distance: %d' % self.highest_distance)
...
class GetPeersLookup(object):
    ...
    def __init__(self, my_id, querier_, max_parallel_queries,
                 ...
                 bt_port=None):
        logging.debug('New lookup (info_hash: %r)' % info_hash)
        self._my_id = my_id
    ...
    def _on_timeout(self, node_):
        logging.debug('TIMEOUT node: %r' % node_)
        #with self._lock:
        self._lock.acquire()
        try:
            self._num_parallel_queries -= 1
    ...
    def _on_error(self, error_msg, node_): 
        logging.debug('ERROR node: %r' % node_)
        #with self._lock:
        self._lock.acquire()
        try:
            self._num_parallel_queries -= 1
        ...
        try:
            while self._num_parallel_queries < self._max_parallel_queries:
                try:
                    ...
                    logging.debug('popped node %r' % node_)
                except(IndexError):
                    logging.debug('no more candidate nodes!')
                    if not self._num_parallel_queries:
                        logging.debug('Lookup DONE')
                        self._announce()
                    return
                if node_.id == self._my_id:
                    ...
                    continue
                self._num_parallel_queries += 1
                logging.debug('sending to: %r, parallelism: %d/%d' %
                    (node_,
    ...
    def _announce(self):
        ...
        for (_, node_, token) in self._announce_candidates:
            logging.debug('announcing to %r' % node_)
