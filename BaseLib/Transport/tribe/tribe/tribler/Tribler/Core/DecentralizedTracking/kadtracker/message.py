...
def bin_to_ip(bin_str):
    ...
    if not bin_str.startswith(IP6_PADDING):
        raise IP6Addr, 'IPv4 and v6 should not be mixed!'
...
def uncompact_addr(c_addr):
    if c_addr[-2:] == '\0\0':
        logging.warning('c_addr: %r > port is ZERO' % c_addr)
        raise AddrError
...
def _uncompact_nodes2(c_nodes):
    ...
    for c_node in c_nodes:
        ...
        try:
            ...
            logging.warning('IPv6 addr in nodes2: %s' % c_node)
        except (AddrError):
            pass
        else:
            node = Node(node_addr, node_id)
...
class IncomingMsg(object):
    ...
    def sanitize_response(self, query):
        ...
        if query in [FIND_NODE, GET_PEERS]:
            ...
            try:
                ...
                if nodes_found:
                    logging.info('Both nodes and nodes2 found')
                nodes_found = True
            except (KeyError):
                pass
        if query == FIND_NODE:
            if not nodes_found:
                logging.warning('No nodes in find_node response')
    ...
    def _sanitize_error(self):
        ...
        if self.error not in [GENERIC_E, SERVER_E, PROTOCOL_E, UNKNOWN_E]:
            logging.info('Unknown error: %s', self.error)
