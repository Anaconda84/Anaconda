...
class OverlayApps:
    ...
    def handleMessage(self,permid,selversion,message):
        ...
        if not self.requestAllowed(permid, message[0]):
            if DEBUG:
                print >> sys.stderr, time.asctime(),'-', "olapps: Message not allowed", getMessageName(message[0])
            return False
        ...
        try:
            if DEBUG:
                ...
                diff = et - st
                if diff > 0:
                    print >> sys.stderr,time.asctime(),'-', "olapps: ",getMessageName(id_),"returned",ret,"TOOK %.5f" % diff
                return ret
            else:
