...
class ExpSmoothRatePredictor(RatePredictor):
    def __init__(self, raw_server, rate_measure, max_rate, alpha = 0.5, max_period = 30, probing_period = 2):
        ...
        if DEBUG: print >>sys.stderr, time.asctime(),'-', "RatePredictor:__init__"
        self.alpha = alpha
    ...
    def predict(self):
        if DEBUG: print >>sys.stderr, time.asctime(),'-', "RatePredictor:predict"
        # self.update()
    ...
    def has_capacity(self):
        if DEBUG: print >>sys.stderr, time.asctime(),'-', "RatePredictor:has_capacity"
#        return False
        # self.update()
        result = None
        if self.value is None:
