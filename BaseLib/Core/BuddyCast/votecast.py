...
class VoteCastCore:
    ...
    def createAndSendVoteCastMessage(self, target_permid, selversion):
        ...
        if selversion < OLPROTO_VER_THIRTEENTH:
            if DEBUG:
                print >> sys.stderr, time.asctime(),'-', "votecast: Do not send to lower version peer:", selversion
            return
                
        votecast_data = self.createVoteCastMessage()
        if len(votecast_data) == 0:
            if DEBUG:
                print >>sys.stderr, time.asctime(),'-', "votecast: No votes there.. hence we do not send"            
            return
        ...
        if DEBUG: print >> sys.stderr, time.asctime(),'-', "votecast: Sending votecastmsg",voteCastMsgToString(votecast_data)
...
#        self.secure_overlay.send(target_permid, data, self.voteCastSendCallback)
    ...
    def createVoteCastMessage(self):
        ...
        if DEBUG: print >> sys.stderr, time.asctime(),'-', "votecast: Creating votecastmsg..."        
        ...
        for record in records:
            ...
            if DEBUG:
                print >>sys.stderr,time.asctime(),'-', "votecast: publisher id",`record[0]`,type(record[0]) 
    ...
    def voteCastSendCallback(self, exc, target_permid, other=0):
        if DEBUG:
            if exc is None:
                print >> sys.stderr,time.asctime(),'-', "votecast: *** msg was sent successfully to peer", show_permid_short(target_permid)
            else:
                print >> sys.stderr, time.asctime(),'-', "votecast: *** warning - error in sending msg to", show_permid_short(target_permid), exc
    ...
    def gotVoteCastMessage(self, recv_msg, sender_permid, selversion):
        ...
        if selversion < OLPROTO_VER_THIRTEENTH:
            if DEBUG:
                print >> sys.stderr, time.asctime(),'-', "votecast: Do not receive from lower version peer:", selversion
            return True
                
        if DEBUG:
            print >> sys.stderr,time.asctime(),'-', 'votecast: Received a msg from ', show_permid_short(sender_permid)
    ...
    def handleVoteCastMsg(self, sender_permid, data):
        ...
        if DEBUG: 
            print >> sys.stderr, time.asctime(),'-', "votecast: Processing VOTECAST msg from: ", show_permid_short(sender_permid), "; data: ", repr(data)
        ...
        if DEBUG:
            print >> sys.stderr,time.asctime(),'-', "votecast: Processing VOTECAST msg from: ", show_permid_short(sender_permid), "DONE; data:"
